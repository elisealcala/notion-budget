        ...
        <TableBody>
          {data.results.map((item: Expense) => 
            <TableRow key={item.id}>
              <TableCell>
                {item.properties['Date'].type === 'date' ? item.properties['Date'].date?.start : ""}
              </TableCell>  
              <TableCell>
                {item.properties['Name'].type === 'title' ? item.properties['Name'].title[0].plain_text : ""}
              </TableCell>  
              <TableCell>
                {item.properties['Amount'].type === 'number' ? item.properties['Amount'].number?.toFixed(2) : ""}
              </TableCell>  
              <TableCell>
                {item.properties['Paid'].type === 'status' ? item.properties['Paid'].status?.name : ""}
              </TableCell>  
              <TableCell>
              {item.properties['Account'].type === 'relation' ? item.properties['Account'].relation[0].id : ""}
              </TableCell>  
              <TableCell>
              {item.properties['Category'].type === 'relation' ? item.properties['Category'].relation[0].id : ""}
              </TableCell>  
            </TableRow>
          )}
        </TableBody>
